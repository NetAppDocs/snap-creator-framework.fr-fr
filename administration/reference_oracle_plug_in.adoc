...
(O|N)
 a| 
Si cette option est définie sur Y lors de l'exécution de la sauvegarde hors ligne d'Oracle, elle utilise le mécanisme d'annulation d'arrêt pour garantir que la base de données s'arrête dans un état cohérent. La procédure d'arrêt pour le paramètre ORACLE_SHUTDOWNABORT est la suivante :

* Arrêt interrompu
* Restriction des startups
* Arrêt immédiat
