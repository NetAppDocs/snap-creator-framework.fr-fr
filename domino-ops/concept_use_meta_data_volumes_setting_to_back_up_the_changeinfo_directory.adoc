...
Vous auriez déjà dû identifier le volume changeinfo comme un volume à sauvegarder lors de la création du fichier de configuration. Pour plus d'informations, consultez l'étape link:task_using_the_gui_to_create_a_configuration_file.html["tâche à l'aide de l'interface graphique pour créer une configuration"].
