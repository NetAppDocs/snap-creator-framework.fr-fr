...
Par défaut, une opération de sauvegarde échouera si une ou plusieurs bases de données sont dans un état incohérent ou corrompu. Vous pouvez forcer le plug-in à poursuivre les opérations de sauvegarde malgré les erreurs, comme décrit à l’étape link:task_using_the_gui_to_create_a_configuration_file.html["tâche à l'aide de l'interface graphique pour créer un fichier de configuration"].
...
Le plug-in Domino ne peut pas fonctionner avec SNAP_TIMESTAMP_ONLY = N. SNAP_TIMESTAMP_ONLY doit être défini sur Y.
 a| 
Pour plus d'informations, voir l'étape link:task_using_the_gui_to_create_a_configuration_file.html["tâche_utiliser_l'interface_graphique_pour_créer_un_fichier_de_configuration"].
